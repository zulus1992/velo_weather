...
| Файл | Назначение |
|---|---|
| `get_weather_forecast.py` | прогноз OpenWeatherMap в плоском виде (`date`, `date_local`, `temp`, `wind_speed`, `wind_speed_kmh`, `condition`, `pop`) + восход и закат по суткам |
| `cycling_rules.py` | правила из xlsx + расчёт вердикта и «окон» времени (только между восходом и закатом) |
| `sun_times.py` | восход и закат офлайн по координатам города (алгоритм NOAA) |
...
* Оценка идёт по светлому времени суток: **кататься раньше восхода и после заката нельзя**.
  Восход и закат считает `sun_times.py` по координатам и часовому поясу города из ответа API
  (офлайн, алгоритм NOAA, точность 1–3 минуты). В сообщении они выводятся строкой
  `• светлое время: 06:58–19:03`, а окна обрезаются по восходу и закату.
...
* Если координат города в ответе нет (или включён `--no-sun`), используется резервный
